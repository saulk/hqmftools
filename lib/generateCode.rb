...
module HQMFToCode
    ...
    class HQMJSonCleanup    
      @@relative_timings = Set.new(['SBS', 'SBE', 'EAS','SAE'])
   ...
   if section['maxDepth'] > 1
     ...
     if (timingchild['type'] != 'date_interval') |
      ...
      @@errfout.puts "mergedatetiminginterval -- section #{section['name']} depth = #{section['maxDepth']} title #{section['title']} ph_id = #{section['ph_id']}"
         ...
         section['value'] =  "dateTimingInterval(#{section['ph_id']}, #{getEmitName(timingchild)}.start, #{getEmitName(timingchild)}.end, #{timingchild['typecode']}, #{low}, #{high} #{lowunits}, #{highunits}, #{lowinclusive}, #{highinclusive} )"
         section['name'] = getNodeName
...
def self.mergeEventListDuringInterval(section)
  ...
  if section['maxDepth'] > 1
    ...
    child = section['children'][0]
    if (section['typecode'] == 'PRCN' || section['typecode'] == 'DURING') &
        ( section['type'] == 'event_list') &
        ( child['typecode'] == 'DURING' ) &
        ( child['type'] == 'date_interval')
        ...
        section['value'] =  "eventsDuring(#{section['ph_id']}, #{getEmitName(child)}.start, #{getEmitName(child)}.end)"
        section['name'] = getNodeName
      end  
    end
  end
...
#        @@errfout.puts "typecode = #{section['typecode']} type = #{section['type']} timing = #{timingchild['typecode']} eltype = #{elchild['type']}"
        ...
        if timingchild['inversionInd'] == "*invert*"
          ...
          b = getEmitName(section)
        else
          b = getEmitName(timingchild)
          a = getEmitName(section)
        end
            section['status'] = 'emit'
...
#    @@errfout.puts "returning...depth = #{section['maxDepth']} and num_merges = #{num_merges}"
  ...
  def self.mergeChildValueSets(section)
    newchildren = []
    newnodename = getNodeName
    newnode = {'name' => newnodename, 'type' => "value", 'level' => '0',  'num_merges' => '0', 'type' => "event_list", 'status' => 'emit', 'pattern' =>'merged_value_sets'}
    mergelist = []
    nameslist = []
    # iterate over children, and build list of merging children
    section['children'].each do |child|
      if child['num_merges'] == 1
        mergelist << child['value']
        nameslist << child['name']
      end
    end
    newnode['value'] = "normalize(#{mergelist.join(', ')})"
    newnode['oldnames'] = nameslist.join(',')
    newchildren << newnode
    # iterate over children, and build list of merging children
    section['children'].each do |child|
      if child['num_merges'] != 1
